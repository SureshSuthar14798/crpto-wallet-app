...
import { 
  ...
  LogOut, Trash2, Smartphone, Shield, HelpCircle, Info,
  CheckCircle2, CreditCard, Wallet, Sparkles, Zap, ChevronLeft
} from 'lucide-react';
      ...
      <motion.div variants={item} className="space-y-4">
         ...
         <Card variant="solid" padding="none">
            <div className="divide-y divide-surface-200/50 dark:divide-white/[0.04]">
               ...
               {/* Notifications */}
               <button onClick={() => setNotifs(!notifs)} className="w-full flex items-center gap-4 px-6 py-5 hover:bg-surface-50 dark:hover:bg-dark-50/20 transition-all text-left">
        ...
        <div className="space-y-2 pt-2">
          {languages.map(l => (
            <button 
              ...
              className={`w-full flex items-center justify-between px-5 py-4 rounded-[20px] transition-all ${
                lang === l.name 
            ...
            >
              <div className="flex items-center gap-4 text-left">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${
                  currency === c.code ? 'bg-emerald-500 text-white' : 'bg-surface-200 dark:bg-dark-200 text-gray-500'
                }`}>
                   {c.symbol}
                </div>
                <div>
