        ...
        <div className="bg-surface-100/80 dark:bg-dark-100/50 p-1.5 rounded-2xl flex relative overflow-hidden backdrop-blur-md border border-surface-200/50 dark:border-white/5">
           {tabs.map((t) => (
             <button
               key={t.id}
               onClick={() => setTab(t.id)}
               className={`relative z-10 flex-1 py-2.5 text-sm font-bold transition-all duration-300 ${
                 tab === t.id ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600'
               }`}
             >
               {tab === t.id && (
                 <motion.div 
        ...
        <div className="space-y-2 max-h-[60vh] overflow-y-auto no-scrollbar pt-2">
          {assets.map((asset) => (
            <button
              ...
              className="w-full flex items-center gap-4 p-4 rounded-[20px] hover:bg-surface-100 dark:hover:bg-dark-50 transition-all group"
            >
